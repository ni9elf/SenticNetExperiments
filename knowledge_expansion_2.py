...
def expand_knowledge():  
    ...
    f_txt = open('Output/2/new_sentic_concepts_1.txt', 'w')
    f_pos_txt = open('Output/2/new_positive_sentic_concepts_1.txt', 'w')
    f_neg_txt = open('Output/2/new_negative_sentic_concepts_1.txt', 'w')
    ...
    csvfile = open('Output/2/new_sentic_concepts_1.csv', 'wb')
    csvfile_pos = open('Output/2/new_positive_sentic_concepts_1.csv', 'wb')
    csvfile_neg = open('Output/2/new_negative_sentic_concepts_1.csv', 'wb')
    ...
    print "Got {} pos noun words".format(pos_counter)
    print "Got {} neg noun words".format(neg_counter)
    #compute Lin similarity measure for all pairs of words in both corpora
    for i in xrange(len(pos_words)):
        if (i%1000 == 0):
            print "Computed similarity measure for {}/{} pos words".format(i, pos_counter)
    ...
    for i in xrange(len(neg_words)):
        if (i%1000 == 0):
            print "Computed similarity measure for {}/{} neg words".format(i, neg_counter)    
    ...
    for word in pos_words:
        if (ctr%1000 == 0):
            print "Processed {}/{} pos words for possible new concepts".format(ctr, pos_counter)
    ...
    for word in neg_words:
        if (ctr%1000 == 0):
            print "Processed {}/{} neg words for possible new concepts".format(ctr, neg_counter)    
        synset = wn.synset(word)
        s_synset = swn.senti_synset(synset.name())
        concept = str(synset.name()[:-5]).lower()     
        #if we have found a new input concept for SenticNet
